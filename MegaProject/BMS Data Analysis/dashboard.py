...
with col3:
    st.metric("Owner Name:", "Krishankant Singh")
st.markdown("---")
...
if status == "Critical":
    st.error(
        f"Temperature Status: CRITICAL ({latest_temp:.1f} °C)"
    )
elif status == "Warning":
    st.warning(
        f"Temperature Status: WARNING ({latest_temp:.1f} °C)"
    )
else:
    st.success(
        f"Temperature Status: NORMAL ({latest_temp:.1f} °C)"
    )
...
if latest_temp > 40:
    alerts.append(
        f"High Battery Temperature: {latest_temp:.1f} °C"
    )
if latest_soc < 20:
    alerts.append(
        f"Low State of Charge: {latest_soc:.1f}%"
    )
if latest_soh < 80:
    alerts.append(
        f"Battery Health Warning: SOH = {latest_soh:.1f}%"
    )
